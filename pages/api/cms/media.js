  ...
  if (req.method === 'POST') {
    ...
    if (req.query.action === 'create') {
      ...
      scope.tryRun(async () => {
        ...
        let folder = await MongoDB.Media.findOne({ _id: scope.req.body.data._id, userID: user._id })
        if (folder && user) {
          // await Promise.all([
          //   MongoDB.Media.deleteMany({ userID: user._id, folderID: folder._id }),
          // ])

          await MongoDB.Media.findOneAndDelete({ userID: user._id, _id: scope.req.body.data._id })
      ...
      scope.tryRun(async () => {
        ...
        let folder = await MongoDB.Media.findOne({ _id: scope.req.body.data._id })
        if (folder && user) {
          // await Promise.all([
          //   MongoDB.Media.deleteMany({ folderID: folder._id }),
          // ])

          await MongoDB.Media.findOneAndDelete({ _id: scope.req.body.data._id })
