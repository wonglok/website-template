  ...
  if (req.method === 'POST') {
    ...
    if (req.query.action === 'create') {
      ...
      scope.tryRun(async () => {
        ...
        let folder = await MongoDB.Posts.findOne({ _id: scope.req.body.data._id, userID: user._id })
        if (folder && user) {
          // await Promise.all([
          //   MongoDB.Posts.deleteMany({ userID: user._id, folderID: folder._id }),
          // ])

          await MongoDB.Posts.findOneAndDelete({ userID: user._id, _id: scope.req.body.data._id })
      ...
      scope.tryRun(async () => {
        ...
        let folder = await MongoDB.Posts.findOne({ _id: scope.req.body.data._id })
        if (folder && user) {
          // await Promise.all([
          //   MongoDB.Posts.deleteMany({ folderID: folder._id }),
          // ])

          await MongoDB.Posts.findOneAndDelete({ _id: scope.req.body.data._id })
